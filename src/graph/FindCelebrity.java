 ...
 class FindCelebrity{
     ...
     private static int findCelebrity(int n,Vector<Vector<Integer> > edgeList){
         ...
         boolean celeb[] = new boolean[n+1];
         Arrays.fill(celeb, true);
         for(int i=1;i<=n;i++) {
            mp.putIfAbsent(i, new HashSet<Integer>());
         }
         for(int i=0;i<edgeList.size();i++) {
        	 int cut = edgeList.get(i).get(0);//4
        	 int add = edgeList.get(i).get(1);//1
        	 celeb[cut] = false;
        	
        	 mp.get(add).add(cut);
         }
         
         for(Entry<Integer,Set<Integer>>e:mp.entrySet()) {
        	 int key = e.getKey();
        	 if(celeb[key] && e.getValue().size() == n-1) {
        		 return key;
        	 }
         }
         return -1;
