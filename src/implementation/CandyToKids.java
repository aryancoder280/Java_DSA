...
public class CandyToKids {
    ...
    static int candyToKids(int n, ArrayList<Integer>goodness){
        ...
        for(int i=0;i<goodness.size();i++) {
            if(i-1>=0) {
                if(goodness.get(i-1)<goodness.get(i) && count[i-1]>=count[i]) {
                	Integer diff = Math.abs(count[i]-count[i-1])+1;
                    count[i] +=diff;
                }
            }
            if(i+1<n) {
            	if(goodness.get(i)>goodness.get(i+1) && count[i]<=count[i+1]) {
            		Integer diff = Math.abs(count[i]-count[i+1])+1;
            		count[i] += diff;
            	}
            }
